...
プロジェクト開始時にこのリストを `{project_name}/temp/TaskList.{timestamp}.md` のようなパスに生成し、各フェーズを通過するごとにステータスを更新して報告します。
