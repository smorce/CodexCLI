...
プロジェクト開始時にこのリストを `{project_name}/temp/TaskList.{timestamp}.md` のようなパスに生成し、各フェーズを通過するごとにチェックボックスを埋めて報告します。
...
[ ] **Step 5: TDDによる実装 (ユーザーまたは開発者担当)**
    ...
    - [ ] 「5.4. マイクロサービス実装チェックリスト」を用いて品質を確認
